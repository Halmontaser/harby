...
""" 
@frappe.whitelist()
def get_linked_tenders(conutact):
	"""Get linked deals for a contact"""

	if not frappe.has_permission("Contact", "read", contact):
		frappe.throw("Not permitted", frappe.PermissionError)

	tenders_names = frappe.get_all(
		"tender Contacts",
		filters={"contact": contact, "parenttype": "tender"},
		fields=["parent"],
		distinct=True,
	)
	...
	# get deals data
	tenders = []
	for d in tenders_names:
		tender = frappe.get_cached_doc(
			"tender",
			d.parent,
			fields=[
				"name",
