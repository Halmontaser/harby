    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div v-for="doc in documents" :key="doc.type" class="document-item">
        ...
        >
          ...
          <FileUploader
     @success="(f) => updateDocumentUrl(f.file_url)"
     @failure="imageAddFailure"
     fileTypes="image/*"
     :upload-args="{
      doctype: 'supplier',docname: 'doc.name',
                        }"
                            >
                                <template
          ...
          <button
            v-if="!doc.uploaded"
            class="flex items-center px-3 py-2 bg-blue-500 text-white rounded"
          ...
          <button
            v-else
            class="flex items-center px-3 py-2 bg-green-500 text-white rounded"
